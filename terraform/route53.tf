...
resource "aws_route53_record" "frontend_alb_route53_validation_record" {
  for_each = {
    for dvo in aws_acm_certificate.frontend_alb_cert.domain_validation_options : dvo.domain_name => {
      name   = dvo.resource_record_name
      record = dvo.resource_record_value
      type   = dvo.resource_record_type
    }
